...
test('real Express playback authorization and Range matrix', async t => {
  ...
  const issued = await request(server, base + '/playback', 'POST', { Authorization: 'Bearer ' + token, Origin: origin });
  ...
  assert.match(issued.headers['set-cookie'][0], /SameSite=None/);
  assert.match(issued.headers['set-cookie'][0], /Secure/);
  assert.match(issued.headers['set-cookie'][0], new RegExp('Path=' + data.url));
  const localIssued = await request(server, base + '/playback', 'POST', { Authorization: 'Bearer ' + token });
  assert.match(localIssued.headers['set-cookie'][0], /SameSite=Lax/);
  assert.doesNotMatch(localIssued.headers['set-cookie'][0], /Secure/);
